...
Then  图片上传成功
And   系统生成缩略图（sm: 150×150 裁切、md: 400×400 等比缩放）
...
**As a** 系统管理员
**I want to** 系统在关键事件发生时自动发送邮件通知
**So that** 相关用户能及时获知账户变动和内容互动
...
Scenario: 密码重置邮件
Given 用户请求密码重置
When  系统生成重置链接（有效期 30 分钟）
Then  通过 Resend 发送包含重置链接的邮件至用户邮箱
And   邮件包含站点名称、用户名、过期时间说明

Scenario: 账号被禁用通知
Given 管理员禁用某用户账号
When  禁用操作完成
Then  系统向该用户邮箱发送账号已被禁用的通知邮件
And   邮件包含禁用原因（如有）和联系管理员的指引

Scenario: 新用户创建通知
Given 管理员创建新用户并生成临时密码
When  用户记录创建成功
Then  系统向新用户邮箱发送欢迎邮件
And   邮件包含临时密码和首次登录修改密码的指引

Scenario: 新评论提交通知
Given 访客或用户在文章下提交评论
When  评论进入待审核队列
Then  系统向文章作者和站点管理员发送新评论通知邮件
And   邮件包含评论者信息、评论内容摘要、审核链接

Scenario: 评论被回复通知
Given 管理员回复了某条评论
When  回复保存成功
Then  系统向原评论者邮箱发送回复通知
And   邮件包含回复内容摘要和查看链接
```

**技术实现要点**
- 邮件发送通过 `pkg/mail` 包（ResendSender / NoopSender）
- 环境变量：`RESEND_API_KEY`、`RESEND_FROM_NAME`、`RESEND_FROM_EMAIL`
- 模板化邮件内容，支持站点名称动态替换
- 发送失败记录 slog 警告，不阻塞主流程
...
**As a** 管理后台用户
**I want to** 看到一个清晰、响应式的管理界面布局
**So that** 我可以高效地导航和操作各功能模块

**验收标准**

1. 管理后台采用侧边栏 + 内容区布局，侧边栏根据用户 RBAC 权限动态渲染菜单项（无权限菜单不显示）
2. 支持暗色模式 / 亮色模式切换（shadcn/ui 主题系统），用户偏好持久化到 localStorage
3. 响应式布局适配 1280px+ 宽屏，侧边栏可折叠
4. 多站点用户可通过头部站点切换器切换当前站点，切换后自动刷新数据
5. 页面导航包含面包屑，显示当前页面层级路径
6. 所有页面实现加载骨架屏（Skeleton）、空状态、错误边界（Error Boundary）三态
7. 操作反馈统一使用 Sonner Toast（成功绿色、错误红色、警告橙色）
8. 国际化支持中文 / 英文切换（react-i18next），语言偏好持久化

**技术实现要点**
- Astro 5 SSR + React 19 Islands Architecture
- 状态管理：Zustand（全局 auth/theme/locale 状态）+ TanStack Query（服务端数据缓存）
- 组件库：shadcn/ui（Radix UI + Tailwind CSS V4）
- 路由守卫：Astro 中间件验证 JWT，未登录重定向至 /login
...
**As a** 系统用户
**I want to** 通过浏览器完成登录、2FA 验证、密码重置和首次安装
**So that** 我可以安全地访问管理后台
...
Given 用户访问 /login
When  输入邮箱和密码并提交
Then  表单进行客户端验证（邮箱格式、密码非空）
And   登录成功后跳转至 Dashboard
And   登录失败显示错误提示（凭证错误 / 账号锁定 / 账号禁用）

Scenario: 2FA 验证页面
Given 用户登录后服务端返回 requires_2fa: true
When  页面跳转至 /login/2fa
Then  显示 TOTP 验证码输入框（6 位数字）
And   支持使用备用码登录（切换入口）
And   验证通过后跳转至 Dashboard

Scenario: 密码重置流程
Given 用户点击"忘记密码"
When  输入注册邮箱并提交
Then  显示"重置邮件已发送"提示
And   用户通过邮件链接访问 /reset-password?token=xxx
And   输入新密码并确认后完成重置
...
When  访问任意页面
Then  自动重定向至 /setup
And   分步表单：语言选择 → 数据库连接测试 → 站点信息 → 管理员账户创建
And   安装完成后锁定向导入口，重定向至登录页
```
...
**As a** 内容编辑者
**I want to** 通过直观的界面管理文章、分类、标签和媒体
**So that** 我可以高效地创建和组织网站内容

**验收标准**

1. **文章列表**：表格展示（标题、状态、分类、作者、发布时间），支持按状态/分类/作者筛选，支持关键词搜索（Meilisearch），支持分页，支持批量操作（删除、变更状态）
2. **文章编辑器**：富文本编辑器（TipTap / BlockNote），支持标题、段落、列表、表格、代码块（语法高亮）、图片/视频嵌入（从媒体库选择）；右侧面板配置：分类、标签、特色图、SEO 元数据、发布状态
3. **自动保存**：编辑器内容每 30 秒自动保存草稿，显示"已保存"/"保存中"状态指示
4. **修订历史**：查看文章修订记录列表，支持两个版本之间的 diff 对比（删除线+高亮），支持一键回滚至历史版本
5. **分类管理**：树形结构展示，支持拖拽重排序，支持新增/编辑/删除（有子分类或关联文章时警告）
6. **标签管理**：标签列表 + 搜索，支持新增/编辑/删除，显示关联文章数量
7. **媒体库**：网格/列表视图切换，支持拖拽上传 + 点击上传，显示缩略图/文件名/大小/类型，支持搜索和筛选，支持批量删除（有引用时警告）

**相关页面**
- `/posts` — 文章列表
- `/posts/new` — 新建文章
- `/posts/:id/edit` — 编辑文章
- `/posts/:id/revisions` — 修订历史
- `/categories` — 分类管理
- `/tags` — 标签管理
- `/media` — 媒体库
...
**As a** 站点管理员
**I want to** 通过管理界面配置站点设置、管理用户和审查系统状态
**So that** 我可以维护站点的正常运营

**验收标准**

1. **用户管理**：用户列表（昵称、邮箱、角色、状态），支持新增用户（分配角色、发送临时密码邮件）、编辑用户信息、禁用/启用用户（仅 Super）
2. **角色权限管理**：角色列表，支持创建自定义角色、编辑角色名称和描述、分配 API 权限和菜单权限，支持权限模板一键应用
3. **站点设置**：站点名称、Logo、域名、描述等基础配置，保存后即时生效，Redis 缓存自动刷新
4. **API Key 管理**：Key 列表（名称、前缀、创建时间、状态），支持创建（完整 Key 仅显示一次）、吊销
5. **审计日志**：只读列表展示（操作者、动作、目标、IP、时间），支持按操作类型/操作者/时间范围筛选，90 天保留期
6. **评论审核**：评论列表（内容、作者、文章、状态），支持批准/标记垃圾/删除操作，支持管理员回复，置顶管理（最多 3 条）
7. **导航菜单管理**：菜单列表，支持菜单项拖拽排序（3 层嵌套），5 种菜单项类型（自定义链接/文章/分类/标签/页面）
8. **重定向管理**：重定向规则列表（源路径、目标路径、类型、命中次数），支持 CSV 导入/导出
9. **多站点管理**（仅 Super）：站点列表（名称、slug、域名、状态），支持新增/编辑/启停站点
...
| 优先级 | 用户故事 |
|--------|----------|
| P0（必须） | US-001, US-002, US-004, US-005, US-006, US-012, US-028, US-029, US-030 |
| P1（重要） | US-003, US-007, US-010, US-011, US-013, US-014, US-015, US-016, US-017, US-019, US-020, US-021, US-022, US-026, US-027, US-031 |
