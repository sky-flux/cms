...
erDiagram
    ...
    %% ============ RELATIONSHIPS ============
    sfc_sites                  ||--o{ sfc_site_user_roles        : "has roles"
    sfc_users                  ||--o{ sfc_site_user_roles        : "assigned to"
...
-- 1. 用户表（全局，所有站点共享）
-- ============================================
-- 注意：V1.0 中的 role 列已移除，角色现通过 sfc_site_user_roles 表按站点分配
...
-- ============================================
-- 3. 站点用户角色表（全局，替代原 sfc_users.role 列）
-- ============================================
-- 一个用户可以在不同站点拥有不同角色（如：站点 A 的 SuperAdmin，站点 B 的 Editor）
CREATE TABLE public.sfc_site_user_roles (
    site_id    UUID       NOT NULL REFERENCES public.sfc_sites(id) ON DELETE CASCADE,
...
CREATE INDEX idx_sfc_sur_user_id ON public.sfc_site_user_roles(user_id);

CREATE TRIGGER trg_sfc_site_user_roles_updated_at
    BEFORE UPDATE ON public.sfc_site_user_roles FOR EACH ROW EXECUTE FUNCTION update_updated_at();

-- ============================================
-- 4. 刷新令牌表（全局，持久化 Refresh Token）
-- ============================================
...
-- ============================================
-- 2. 创建初始管理员（密码：Admin@123456，bcrypt cost=12）
-- ============================================
-- 仅限开发/测试环境使用，生产环境应通过安装向导或 CLI 命令创建
-- 注意：sfc_users 表不再包含 role 列，角色通过 sfc_site_user_roles 分配
...
-- 3. 分配管理员角色到默认站点
-- ============================================
INSERT INTO public.sfc_site_user_roles (site_id, user_id, role)
SELECT s.id, u.id, 'superadmin'
FROM public.sfc_sites s, public.sfc_users u
WHERE s.slug = 'default_site' AND u.email = 'admin@example.com'
...
# 站点角色缓存
# ============================================
site:{slug}:role:{user_id}               TTL=300s            -- 用户在该站点的角色
...
```
migrations/
├── 20260201000001_create_enums_and_functions.go    -- 枚举类型 + update_updated_at() 函数
├── 20260201000002_create_public_schema.go          -- sfc_users, sfc_sites, sfc_site_user_roles, sfc_refresh_tokens, sfc_user_totp, sfc_configs
├── 20260201000003_create_site_template.go          -- 定义站点 schema DDL 模板函数
├── 20260224000001_add_comments_table.go            -- 站点迁移：添加 sfc_site_comments 表
├── 20260224000002_add_menus_tables.go              -- 站点迁移：添加 sfc_site_menus + sfc_site_menu_items 表
├── 20260224000003_add_redirects_table.go           -- 站点迁移：添加 sfc_site_redirects 表
├── 20260224000004_add_preview_tokens_table.go      -- 站点迁移：添加 sfc_site_preview_tokens 表
├── 20260224000005_create_user_totp.go              -- 全局迁移：创建 public.sfc_user_totp
└── ...
```
