...
| Super 角色全局操作 | Super 角色可绕过站点范围执行管理操作（站点 CRUD、用户 2FA 管理），通过 RBAC 中间件动态控制 |
...
> **V2.0 变更**：`role` 字段已从 JWT Claims 中移除。角色是全局的，存储在 `public.sfc_user_roles` 表中。每次请求时通过 RBAC 中间件从 `sfc_user_roles JOIN sfc_roles` 解析角色 slugs（两级 Redis 缓存：L1 用户角色 TTL=300s + L2 角色 API 集 TTL=600s），确保角色变更在缓存 TTL 内生效，无需重新签发 Token。
...
**存储机制**：
- 客户端：Refresh Token 存储在 httpOnly + Secure + SameSite=Lax Cookie 中
...
func (s *AuthService) RefreshToken(ctx context.Context, rawToken string) (*TokenPair, error) {
    ...
    // 3. 颁发新的 Token 对（Access Token 不含 role，角色通过 RBAC 中间件 per-request 解析）
...
sequenceDiagram
    ...
    rect rgb(255, 248, 240)
        ...
        alt Token 在黑名单中
            ...
            Redis-->>Gin: 0
            Gin->>Redis: GET user:{user_id}:roles (L1 缓存)
            alt 角色缓存命中
                Redis-->>Gin: {slugs: ["admin"], role_ids: ["..."]}
            else 角色缓存未命中
                Gin->>PG: 从 sfc_user_roles JOIN sfc_roles 查询角色 slugs
                PG-->>Gin: {slugs: ["admin"], role_ids: ["..."]}
                Gin->>Redis: SET user:{user_id}:roles EX 300
            end
            Gin->>Redis: GET role:{role_id}:api_set (L2 缓存)
            alt API 集缓存命中
                Redis-->>Gin: ["GET:/api/v1/posts", ...]
            else API 集缓存未命中
                Gin->>PG: 从 sfc_role_apis JOIN sfc_apis 查询角色 API 权限集
                PG-->>Gin: ["GET:/api/v1/posts", ...]
                Gin->>Redis: SET role:{role_id}:api_set EX 600
            end
            Gin->>Gin: RBAC 权限校验
...
当用户丢失认证设备且备用码耗尽时，Super 角色用户可通过 `DELETE /api/v1/auth/2fa/users/:user_id` 强制禁用 2FA：
- 删除 `public.sfc_user_totp` 记录
- 吊销该用户所有 Refresh Token
- 创建包含禁用原因的审计日志条目
- 仅 Super 角色可执行此操作（通过 RBAC 中间件 + `sfc_role_apis` 动态匹配）

---

## 5. RBAC 权限控制

### 5.1 全局角色模型 + 动态 API 权限

角色是**全局**的，存储在 `public.sfc_roles` 表中。用户-角色分配通过 `public.sfc_user_roles` 表管理，API 级权限通过 `public.sfc_role_apis` 动态配置。

**核心表职责**：

| 表名 | 职责 |
|------|------|
| `sfc_roles` | 定义角色（slug、名称、描述、是否内置） |
| `sfc_user_roles` | 全局用户-角色分配（无 site_id） |
| `sfc_apis` | API 端点注册表（method、path、名称、分组） |
| `sfc_role_apis` | 角色-API 权限映射 |
| `sfc_admin_menus` | 后台菜单项定义 |
| `sfc_role_menus` | 角色-菜单可见性映射 |
| `sfc_role_templates` | 权限模板定义 |
| `sfc_role_template_apis` | 模板-API 权限映射 |
| `sfc_role_template_menus` | 模板-菜单权限映射 |

**内置角色**（`built_in = true`，不可删除）：

| 角色名称 | Slug | 说明 |
|---------|------|------|
| 超级管理员 | `super` | 拥有所有权限，不可修改/删除，RBAC 中间件直接放行 |
| 管理员 | `admin` | 站点管理权限，不可删除 |
| 编辑 | `editor` | 内容创建与编辑权限，不可删除 |
| 查看者 | `viewer` | 只读访问权限，不可删除 |

系统支持通过管理界面创建**自定义角色**，自定义角色可自由配置 API 权限和菜单可见性。
...
```
请求 → 提取 JWT user_id
→ 查询用户角色 slugs（两级 Redis 缓存）
  L1: GET user:{user_id}:roles → 缓存命中则使用
  L1 未命中 → SELECT FROM sfc_user_roles JOIN sfc_roles → 写入 L1 缓存 TTL=300s
→ 若 slugs 包含 "super" → 直接放行（短路）
→ 否则查询角色 API 权限集（L2 缓存）
  L2: GET role:{role_id}:api_set → 缓存命中则使用
  L2 未命中 → SELECT FROM sfc_role_apis JOIN sfc_apis → 写入 L2 缓存 TTL=600s
→ 合并所有角色的 API 集，检查当前 method:path 是否匹配
→ 命中则放行，否则 403 Forbidden
```
...
| 缓存层级 | Redis 键 | TTL | 内容 |
|---------|----------|-----|------|
| L1（用户角色） | `user:{user_id}:roles` | 300s | `{slugs: ["admin"], role_ids: ["uuid..."]}` |
| L2（角色 API 集） | `role:{role_id}:api_set` | 600s | `["GET:/api/v1/posts", "POST:/api/v1/posts", ...]` |
| 菜单树 | `user:{user_id}:menu_tree` | 300s | 合并后的菜单 JSON |

**API 注册表同步**：

应用启动时，`Registry.SyncRoutes()` 扫描所有 Gin 路由并 upsert 到 `sfc_apis` 表，已删除的路由自动标记为 `status=false`，确保 API 端点注册表与代码保持一致。

### 5.3 权限矩阵

内置角色的默认权限通过 `sfc_role_templates` + `sfc_role_template_apis` 在迁移 seed 阶段预配置。管理员可通过管理界面调整非 super 角色的 API 权限。

以下为内置角色的**默认**权限矩阵（可通过 `sfc_role_apis` 动态调整）：
...
> **注意**：Super 角色不依赖 `sfc_role_apis` 表 — RBAC 中间件检测到 `"super"` slug 后直接放行，无需查询 API 权限。

### 5.4 角色缓存安全

| 安全关注点 | 防御措施 |
|-----------|---------|
| 过期角色缓存 | L1 TTL=300s，L2 TTL=600s；角色或权限变更时强制失效对应层缓存 |
| 无角色 = 无访问 | 用户无 `sfc_user_roles` 条目 → RBAC 中间件返回 403 |
| 用户角色变更 | 变更 `sfc_user_roles` 时立即删除 `user:{user_id}:roles` + `user:{user_id}:menu_tree` |
| 角色权限变更 | 变更 `sfc_role_apis` 时立即删除 `role:{role_id}:api_set` |
| Super 角色保护 | Super 角色标记为 `built_in = true`，不可修改 slug 或删除 |
| API 注册表一致性 | 应用启动时 `Registry.SyncRoutes()` 自动同步路由到 `sfc_apis`，过期路由自动禁用 |
...
5. 创建站点 Schema + 所有表
6. 创建管理员用户
7. 创建 sfc_user_roles 条目（分配 super 角色）
...
| JWT Claims 不包含 role（角色通过 RBAC 中间件 per-request 解析） | ☐ |
...
| 检查项 | 状态 |
|--------|------|
| 角色通过 `sfc_roles` 表定义，内置角色 `built_in = true` 不可删除 | ☐ |
| Super 角色 slug 不可修改，RBAC 中间件检测到 `"super"` 直接放行 | ☐ |
| 用户角色分配通过 `sfc_user_roles` 全局表管理 | ☐ |
| API 权限通过 `sfc_role_apis` 动态配置，非硬编码 | ☐ |
| 两级 Redis 缓存：L1 用户角色 TTL=300s，L2 角色 API 集 TTL=600s | ☐ |
| 角色变更时 L1 缓存立即失效（`user:{user_id}:roles` + `user:{user_id}:menu_tree`） | ☐ |
| 权限变更时 L2 缓存立即失效（`role:{role_id}:api_set`） | ☐ |
| API 注册表（`sfc_apis`）启动时通过 `Registry.SyncRoutes()` 自动同步 | ☐ |
| 已删除路由在 `sfc_apis` 中自动标记为 `status=false` | ☐ |
| 角色模板（`sfc_role_templates`）内置模板不可删除 | ☐ |
