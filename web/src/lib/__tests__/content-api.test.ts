...
describe('postsApi', () => {
  it('list calls GET with query params', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith(
      '/v1/site/posts?page=1&per_page=20&status=draft',
      undefined,
    );
  ...
  it('list omits undefined params', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith(
      '/v1/site/posts?page=1&per_page=20',
      undefined,
    );
  });
  ...
  it('get calls GET with id', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/v1/site/posts/abc-123', undefined);
  });
  ...
  it('create calls POST', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/v1/site/posts', body, undefined);
  });
  ...
  it('update calls PUT with version', async () => {
    ...
    expect(api.put).toHaveBeenCalledWith('/v1/site/posts/abc-123', body, undefined);
  });

  it('publish calls POST to publish endpoint', async () => {
    vi.mocked(api.post).mockResolvedValue({ success: true, data: {} });
    await postsApi.publish('abc-123');
    expect(api.post).toHaveBeenCalledWith('/v1/site/posts/abc-123/publish', undefined, undefined);
  });

  it('getRevisions calls GET', async () => {
    vi.mocked(api.get).mockResolvedValue({ success: true, data: [] });
    await postsApi.getRevisions('abc-123');
    expect(api.get).toHaveBeenCalledWith('/v1/site/posts/abc-123/revisions', undefined);
  });
  ...
  it('rollback calls POST with revision id', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith(
      '/v1/site/posts/abc-123/revisions/rev-456/rollback',
      undefined,
      undefined,
    );
  });
...
describe('categoriesApi', () => {
  it('tree calls GET', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/v1/site/categories', undefined);
  });
  ...
  it('delete calls DELETE with id', async () => {
    ...
    expect(api.delete).toHaveBeenCalledWith('/v1/site/categories/cat-1', undefined);
  });
});
...
describe('tagsApi', () => {
  it('list calls GET with query params', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith(
      '/v1/site/tags?page=1&per_page=20&q=go',
      undefined,
    );
  });
  ...
  it('suggest calls GET with query', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/v1/site/tags/suggest?q=go', undefined);
  });
});
...
describe('mediaApi', () => {
  ...
  it('delete calls DELETE with force param', async () => {
    ...
    expect(api.delete).toHaveBeenCalledWith('/v1/site/media/m-1?force=true', undefined);
  });
  ...
  it('updateMeta calls PUT', async () => {
    ...
    expect(api.put).toHaveBeenCalledWith('/v1/site/media/m-1', { alt_text: 'new alt' }, undefined);
  });
});
